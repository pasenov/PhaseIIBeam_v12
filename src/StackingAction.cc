...
StackingAction::ClassifyNewTrack(const G4Track* track)
{
  ...
  //geometry parameters
  G4double Det1SizeZ, Det2SizeZ, Dist, Strip1Depth, Strip1Length, Strip2Depth, Strip2Length, StripDist, StripWidth, xAbs;
  G4int div;
	...
	if ((z_prim >= -Dist/2 - Det1SizeZ) && (z_prim <= -Dist/2 - Det1SizeZ + Strip1Depth) && (x_prim >= (-Strip1Length/2)) && (x_prim <= Strip1Length/2)) {
	    ...
	    if (y_prim == 0)   {
		  ...
		  stripNo = totalNbStrips/2 + div;
	       ...
	       //<< div
               //<< G4endl;
	       fRunaction->AddNbElectronsStrip1(stripNo);
            }
        }
      }
	...
	if ((z_prim >= Dist/2) && (z_prim <= (Dist/2 + Strip2Depth))&& (x_prim >= (-Strip2Length/2)) && (x_prim <= Strip2Length/2)) {
	    ...
	    if (y_prim == 0)   {
		  ...
		  stripNo = totalNbStrips/2 + div;
	       ...
	       //<< div
               //<< G4endl;
	       fRunaction->AddNbElectronsStrip2(stripNo);
            }
        }
      }
