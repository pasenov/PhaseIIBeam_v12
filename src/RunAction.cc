...
void RunAction::BeginOfRunAction(const G4Run*)
{
  ...
  for (G4int k = 0; k < 1016; k = k + 1) {
    fNbElectronsStrip1[k] = 0.;
    fNbElectronsStrip2[k] = 0.;
  }
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  for (G4int i = 68; i < 76; i = i + 1) {
    analysisManager->FillH1(i, fNbElectronsStrip1[i+437]);
  }
  for (G4int j = 76; j < 84; j = j + 1) {
    analysisManager->FillH1(j, fNbElectronsStrip2[j+429]);
  }
  if ( analysisManager->IsActive() ) {
